...
class ViewActions extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch("http://localhost:5000/GetActionByEmail", {
      method: "POST",
    ...
    if (isuserassignee) {
      fetch("http://localhost:5000/GetResponseType", {
        method: "Get",
      })
        .then((response) => response.json())
        .then((response) => {
      ...
      fetch("http://localhost:5000/GetActionStatus", {
        method: "Get",
      })
        .then((response) => response.json())
        .then((response) => {
  ...
  handleSubmit = (event) => {
    ...
    fetch("http://localhost:5000/CreateActionNotes", requestOptions).then(
      function (response) {
  ...
  submitResponseType = (event) => {
    const requestOptions = {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action_id: event.target.getAttribute("actionid"),
        response_type_id: event.target.value,
      }),
    };
    fetch(
      "http://localhost:5000/UpdateActionResponseType",
      requestOptions
  ...
  updateActionStatus = (event) => {
    ...
    fetch("http://localhost:5000/UpdateActionStatus", requestOptions).then(
      function (response) {
        // console.log(response)
        return response.json();
      }
        ...
        <table>
          <thead>
            <tr>
              ...
              {this.state.role != 1 ? <th>Response Type</th> : <th>Status</th>}
              <th>Response note</th>
              <th>Edit</th>
            </tr>
          </thead>
          <tbody>
            {this.state.actionList.map((action) => (
              <tr key={action.Id}>
                ...
                {this.state.role != 1 ? (
                  <td style={{ width: "100%", border: "1px solid" }}>
                    <select
                      ...
                      actionid={action.id}
                      value={action.ResponseType}
                      onChange={(e) => this.submitResponseType(e)}
                    >
                  ...
                  <td style={{ width: "100%", border: "1px solid" }}>
                    <select
                      ...
                      actionid={action.id}
                      value={action.Status}
                      onChange={(e) => this.updateActionStatus(e)}
                    >
                ...
                <td>
                  {this.state.actionNotesList
                    .filter((note) => note.id == action.id)
                    .map((note, index) => (
                      <p>{note.comment == null ? "" : note.comment}</p>
                    ))}
                </td>
